    ...
    for modelo in _MODELOS:
        for _ in range(2):
            try:
                ...
                time.sleep(1.0)
    return {"estado": "ERROR", "motivo": f"IA no disponible: {ultimo_error}"}


def _parsear(texto):
    """El modelo suele devolver JSON limpio, pero blindamos contra fences y ruido."""
    if not texto:
        return {"estado": "ERROR", "motivo": "Respuesta vacia"}
    ...
    return {"estado": _normalizar_estado(limpio), "motivo": limpio[:80]}
